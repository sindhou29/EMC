...
//			this.conn.commit();
	...
	public void readSettlementData(SettRunPkg runPackage, SettlementRunParams params, SettlementData data)
		...
		try {
	        ...
	        if ("PF".contains(settRunType)) {
	            emcAdmStr = UtilityFunctions.trimZeroDecimal(data.getGlobal().getCmwhEmcAdmRounded().toPlainString());
	            psoAdmStr = UtilityFunctions.trimZeroDecimal(data.getGlobal().getCmwhPsoAdmRounded().toPlainString());
	            ...
	            totalGmee = UtilityFunctions.trimZeroDecimal(data.getGlobal().getTotalGmee().toPlainString());
	            totalGmef = UtilityFunctions.trimZeroDecimal(data.getGlobal().getTotalGmef().toPlainString());
	            totalLmee = UtilityFunctions.trimZeroDecimal(data.getGlobal().getTotalLmee().toPlainString());
	            totalLmef = UtilityFunctions.trimZeroDecimal(data.getGlobal().getTotalLmef().toPlainString());
	            totalNmea = UtilityFunctions.trimZeroDecimal(data.getGlobal().getTotalNmea().toPlainString());
	...
	/*	            col2 = recinterval.getFssc();
					...
					stmt = conn.prepareStatement(sqlCommand1);
					stmt.setString(1, Integer.valueOf(recinterval.getPeriodId()).toString());
					stmt.setString(2, col2.toString());
					stmt.setString(3, BigDecimal.ZERO.toString());
					stmt.setString(4, settRunId);
					if (col2.signum() < 0) {
...
/*		        // For Forward Sales Implementation FORWARD SALES HEADER
		        if (sortForwardSales1.containsKey(sacId) == true) {
		            seq = seq + 1;
					stmt = conn.prepareStatement(sqlCmd1ForwardSales);
					stmt.setInt(1, seq);
...
/*		                // Forward Sales Implemenation Begin [ITSM-16708]
		                for (Fsc recforwardsales : sortForwardSales) {
		                    ...
		                    forwardsales_account = recforwardsales.getAccountId();	//forwardSalesLine[3];
		                    forwardsales_number = recforwardsales.getPeriodId();	//forwardSalesLine[4];
	                        ...
	                        if (sacId.equals(forwardsales_account) == true && Integer.parseInt(interval_number) == Integer.parseInt(forwardsales_number)) {
		                        ...
		                        // forwardSalesLine[0] = "";
		                        // }
	            				stmt = conn.prepareStatement(sqlCmd2ForwardSales);
	            				stmt.setInt(1, seq);
	            				stmt.setString(2, Integer.valueOf(interval_number).toString());
		                        if (sac.isMsslAccount() == false) {
		                            // Account is not MSSL
	                            	if (recforwardsales.getFsp().signum() == 0 && recforwardsales.getFsq().signum() == 0) {
			            				...
			            				stmt.setString(3, recforwardsales.getFsrp().toString());
			            				stmt.setString(4, recforwardsales.getFsp().toString());
			            				stmt.setString(5, recforwardsales.getFsq().toString());
			            				stmt.setString(6, recforwardsales.getFssc().toString());
			            				stmt.setString(7, recforwardsales.getContractName());
			            				stmt.setString(8, "");
	                            	}
		                        } else {
		                            // Account is MSSL
		            				stmt.setString(3, recinterval.getFsrpk().toString());
		            				stmt.setString(4, recforwardsales.getFsp().toString());
		            				stmt.setString(5, mkt.getFsq().toString());
		            				stmt.setString(6, recinterval.getFssck().toString());
		            				stmt.setString(7, recforwardsales.getContractName());
		            				stmt.setString(8, UtilityFunctions.formatDecimal(recinterval.getFssck()));
		                        }
...
/*		        //col2footerForwardSales = accountLine[122];
		        ...
		        if (sac.getFssc().signum() == 0) {
		        	...
		        	col2footerForwardSales = UtilityFunctions.formatDecimal(sac.getFssc());
		        }
*/
...
/*		        // FORWARD SALES FOOTER - Added for FSC Implementation
		        if (sortForwardSales1.containsKey(sacId) == true) {
			        seq = seq + 1;
					stmt = conn.prepareStatement(sqlCmd3ForwardSales);
					stmt.setInt(1, seq);
					stmt.setString(2, col2footerForwardSales);
					stmt.setString(3, settRunId);
...
/*		        // //FSC Implementation - Begin [ITSM-16708]
				...
				stmt.setString(3, recaccount.getFssc().toString());
				stmt.setString(4, "0");
				stmt.setString(5, "0");
				stmt.setString(6, recaccount.getTotalFssc().toString());
				stmt.setString(7, settRunId);
				stmt.setString(8, sacId);
				stmt.setString(9, sacVersion);
				stmt.setString(10, settDate);
...
/*		            // //FSC Implementation - Begin [ITSM-16708]
					...
					stmt.setInt(1, seq);
					stmt.setString(2, "FSSC - Forward Sales Contract Settlement Credit");
					stmt.setString(3, recMp.getFssc().toString());
					stmt.setString(4, recMp.getIpGstFssc().toString());
					stmt.setString(5, recMp.getOpGstFssc().toString());
					stmt.setString(6, recMp.getTotalFssc().toString());
					stmt.setString(7, settRunId);
					stmt.setString(8, mpId);
					stmt.setString(9, sacVersion);
					stmt.setString(10, settDate);
